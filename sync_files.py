...
api = 'https://foobar.ru/api/'
user, group = pwd.getpwuid(os.getuid()).pw_name, grp.getgrgid(pwd.getpwuid(os.getuid()).pw_gid).gr_name

def get_hosts(hostname):
    all_hosts = []
    try:
        host_groups = [group['name'] for group in json.loads(requests.get(api + \
                'hosts2groups/{}?format=json'.format(hostname)).text)]
    except Exception as e:
        host_groups = []
        logging.warning('{}: {}'.format(e.__class__.__name__, e))
    if host_groups:
        for group in host_groups:
            all_hosts.extend([host['fqdn'] for host in json.loads(requests.get(api + \
                    'groups2hosts/{}?format=json'.format(group)).text)])
...
def get_config(*args):
    ...
    if local:
        ...
        try:
            config = yaml.load((requests.get('https://git.foobar.ru/sync_files/config.yml?raw').text))
